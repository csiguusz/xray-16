...
void CCharacterInfo::InitSpecificCharacter(shared_str new_id)
{
    ...
    if (Rank().value() == NO_RANK) {
        ...
        {
            const int rank = ::Random.randI(m_SpecificCharacter.RankDef().min, m_SpecificCharacter.RankDef().max);
            SetRank(rank);
        }
        else
        {
    ...
    if (Reputation().value() == NO_REPUTATION)
    {
        ...
        {
            const int rep = ::Random.randI(m_SpecificCharacter.ReputationDef().min, m_SpecificCharacter.ReputationDef().max);
            SetReputation(rep);
        }
        else
        {
